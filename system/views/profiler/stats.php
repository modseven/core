...
<div class="modseven">
    <?php foreach (Profiler::groups() as $group => $benchmarks): ?>
        <table class="profiler">
            <tr class="group">
                ...
                <td class="time" colspan="4"><?php echo number_format($group_stats[$group]['total']['time'], 6) ?> <abbr
                ...
                <tr class="mark time">
                    ...
                    <?php foreach ($group_cols as $key): ?>
                        <td class="<?php echo $key ?>">
                            <div>
                                <div class="value"><?php echo number_format($stats[$key]['time'], 6) ?> <abbr
                                            title="seconds">s</abbr></div>
                                <?php if ($key === 'total'): ?>
                                    <div class="graph"
                                         style="left: <?php echo $group_stats[$group]['max']['time'] ? max(0, 100 - $stats[$key]['time'] / $group_stats[$group]['max']['time'] * 100) : '0' ?>%"></div>
                                <?php endif ?>
                            </div>
                        </td>
                    <?php endforeach ?>
                    ...
                    <?php foreach ($group_cols as $key): ?>
                        <td class="<?php echo $key ?>">
                            <div>
                                ...
                                <?php if ($key === 'total'): ?>
                                    <div class="graph"
                                         style="left: <?php echo $group_stats[$group]['max']['memory'] ? max(0, 100 - $stats[$key]['memory'] / $group_stats[$group]['max']['memory'] * 100) : '0' ?>%"></div>
                                <?php endif ?>
                            </div>
                        </td>
                    <?php endforeach ?>
    ...
    <table class="profiler">
        ...
        <tr class="final mark time">
            <th class="name" rowspan="2"
                scope="rowgroup"><?php echo I18n::get('Application Execution') . ' (' . $stats['count'] . ')' ?></th>
            <?php foreach ($application_cols as $key): ?>
                <td class="<?php echo $key ?>"><?php echo number_format($stats[$key]['time'], 6) ?> <abbr
                            title="seconds">s</abbr></td>
            <?php endforeach ?>
        </tr>
